...
--]]
--require('common')
solar_term = require("huma/lib/solar_term")
...
function GanZhiLi:setTime(t)
    ...
    --end
    --先取公历今年的干支
    self.jqs = solar_term.solar_terms_of_solar_year(self.tday.year)
    ...
    if self.ganZhiYearNum ~= self.tday.year then
        ...
        self.jqs = solar_term.solar_terms_of_solar_year(self.ganZhiYearNum)
    end
...
local function parse_time(time)
    -- 解析日期时间字符串
    local year, month, day, hour, min, sec = string.match(time, "(%d+)-(%d+)-(%d+) (%d+):(%d+):(%d+)")
    -- 转换为数字类型
...
-- 干支历
local function lunar_ganzhi_date(time)
    local x, yidx, midx, didx, hidx
    y = tostring(y)
    x = GanZhiLi:new()
    x:setTime(os.time(parse_time(time)))
    yidx = x:getYearGanZhi()
    midx = x:getMonGanZhi()
    didx = x:getDayGanZhi()
    hidx = x:getHourGanZhi()
    GzData = get60JiaZiStr(yidx) ..
        '年' .. get60JiaZiStr(midx) .. '月' .. get60JiaZiStr(didx) .. '日'
    return GzData
end

-- 帶生肖的干支历
local function lunar_ganzhi_date_with_zodiac(time)
    local x, yidx, midx, didx, hidx
    y = tostring(y)
    x = GanZhiLi:new()
    x:setTime(os.time(parse_time(time)))
    ...
    didx = x:getDayGanZhi()
    hidx = x:getHourGanZhi()
    GzData = get60JiaZiStr(yidx) .. '（' .. zodiacs[x:getYearZhi()] .. '）' ..
        '年' .. get60JiaZiStr(midx) .. '月' .. get60JiaZiStr(didx) .. '日'
    return GzData
end

local function lunar_ganzhi_time(time)
    local x, yidx, midx, didx, hidx
    y = tostring(y)
    x = GanZhiLi:new()
    x:setTime(os.time(parse_time(time)))
...
return {
    ganzhi_date = lunar_ganzhi_date,
    ganzhi_date_with_zodiac = lunar_ganzhi_date_with_zodiac,
    ganzhi_time = lunar_ganzhi_time,
}
