...
local function bin2hex(s)
	local hex = ''
	for i = 1, #s, 4 do
		local bin = s:sub(i, i + 3)
		local dec = tonumber(bin, 2)
		hex = hex .. string.format('%x', dec)
	end
	return hex
end
...
-- 农历数据
local LunarData = { "AB500D2", "4BD0883",
...
local function Date2LunarDate(Gregorian)
	...
	--获取两百年内的农历数据
	Pos = Year - 1900 + 2
	Data0 = LunarData[Pos - 1]
	Data1 = LunarData[Pos]
	--判断农历年份
	local tb1 = Analyze(Data1)
	MonthInfo = tb1[1]
	LeapInfo = tb1[2]
	Leap = tonumber(tb1[3]) -- using tonumber only to disable the warning of unmatched types
	Newyear = tb1[4]
	Date1 = Year .. Newyear
	Date2 = Gregorian
	...
	LunarYear = cTianGan[math.fmod(LYear - 4, 10) + 1] ..
		cDiZhi[math.fmod(LYear - 4, 12) + 1] .. '（' .. cShuXiang[math.fmod(LYear - 4, 12) + 1] .. "）" ..
		"年" .. LunarMonth .. cDayName[LDay]
	--print(LunarYear)
	return LunarYear
end
...
return {
	solar2lunar = Date2LunarDate,
	lunar2solar = LunarDate2Date,
}
